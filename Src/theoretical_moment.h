...
class TheoreticalMoment {
 public:
  ...
  std::complex<double> spectralMoment(double s0) {
    ...
    std::complex<double> contourIntegral =
        Numerics::Integrate(d0Gamma, std::complex<double>(0., 0.),
                            std::complex<double>(2*M_PI, 0.));
  ...
  std::complex<double> D0(std::complex<double> s) {
    ...
    std::complex<double> d0 = constants_.getNc()/12./pow(M_PI, 2)
 ...
 private:
  ...
  double alphaMu(double mu) {
    ...
    std::cout << "alphaSTau, sTau" << Constants::kAlphaSTau/M_PI
              << " \t " << Constants::kSTau << std::endl;
    return alpha_s/M_PI;
  }
